...
module.exports.uint32 = {
	write: function (u, data, path) {
		...
		data.writeUInt32LE(u)
	},
	read: function (state) {
		return state.readUInt32LE()
	}
}
...
module.exports.uint16 = {
	write: function (u, data, path) {
		...
		data.writeUInt16LE(u)
	},
	read: function (state) {
		return state.readUInt16LE()
	}
}
...
const StaticBuffer = (size) => () => {
	return {
		write: function (B, data, path) {
			if (!Buffer.isBuffer(B)) {
				throw new TypeError('Expected a Static Buffer at ' + path + ', got ' + B)
			}
			// exports.uint.write(B.length, data, path)
			data.appendBuffer(B)
		},
...
module.exports.StaticBuffer = StaticBuffer

const Block = (subSchema, nBlockSizeBytes) => () => {
    return {
        write: function (value, data, path) {
            const buffer = subSchema.encode(value)
            const blockSize = buffer.length
            data.writeUIntX(blockSize, nBlockSizeBytes)
            data.appendBuffer(buffer)
        },
        read: function (state) {
            var length = state.readUIntX(nBlockSizeBytes)
            const buffer = state.readBuffer(length)
            try {
	            return subSchema.decode(buffer)        		
            }
            catch(error) {
            	console.log(error)
            }
...
const EnumBlock = (key, subSchemaList, subEnumList, nBlockSizeBytes, nEnumBytes, enumOffset) => () => {
    return {
        write: function (value, data, path) {
        	const enumValue = value[key]
        	const found = subEnumList.indexOf(enumValue)
        	if(found > -1 && subSchemaList[found]) {
        		const subSchema = subSchemaList[found]
	            const buffer = subSchema.encode(value)
	            const blockSize = buffer.length
	            data.writeUIntX(blockSize, nBlockSizeBytes)	            	
	            data.appendBuffer(buffer)
        ...
        read: function (state) {
        	let enumValue = state.peekUIntX(nEnumBytes, enumOffset)
        	const found = subEnumList.indexOf(enumValue)
        	if(found > -1 && subSchemaList[found]) {
        		const subSchema = subSchemaList[found]
	            var length = state.readUIntX(nBlockSizeBytes)
...
module.exports.EnumBlock = EnumBlock
